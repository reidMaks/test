...
resource "kubernetes_deployment" "mosquitto" {
  ...
  spec {
    # Брокер більше не потрібен: датчик HTRAM його не використовує. Скейлимо в
    # нуль, а не видаляємо -- конфіг, ConfigMap, PVC і опис вище лишаються в
    # репозиторії як історія рішення. Щоб повернути брокер, достатньо 1.
    replicas = 0
